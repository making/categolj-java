 ...
 * </p>
 * <p>
 * A typical example is to use this filter to allow cross-domain <a
 * href="http://cometd.org">cometd</a> communication using the standard long
 ...
 * to access the resources. Default value is <b>*</b>, meaning all origins</li>
 ...
 * <b>X-Requested-With</b></li>
 * <li><b>preflightMaxAge</b>, the number of seconds that preflight requests can
 * be cached by the client. Default value is <b>1800</b> seconds, or 30 minutes</li>
...
public class CrossOriginFilter implements Filter {
	...
	private static final List<String> SIMPLE_HTTP_METHODS = Arrays.asList(
			"GET", "POST", "HEAD");
	...
	public void init(FilterConfig config) throws ServletException {
		...
		String allowedCredentialsConfig = config
				.getInitParameter(ALLOWED_CREDENTIALS_PARAM);
		...
		logger.debug("Cross-origin filter configuration: " + ALLOWED_ORIGINS_PARAM
				+ " = " + allowedOriginsConfig + ", " + ALLOWED_METHODS_PARAM
				+ " = " + allowedMethodsConfig + ", " + ALLOWED_HEADERS_PARAM
				+ " = " + allowedHeadersConfig + ", " + PREFLIGHT_MAX_AGE_PARAM
	...
	private void handle(HttpServletRequest request,
		...
		if (origin != null && isEnabled(request)) {
			if (originMatches(origin)) {
				...
				logger.debug("Cross-origin request to " + request.getRequestURI()
						+ " with origin " + origin
	...
	private boolean isMethodAllowed(HttpServletRequest request) {
		...
		logger.debug("{} is {}", ACCESS_CONTROL_REQUEST_METHOD_HEADER,
				accessControlRequestMethod);
		boolean result = false;
		if (accessControlRequestMethod != null) {
			result = allowedMethods.contains(accessControlRequestMethod);
		}
		logger.debug("Method {} is" + (result ? "" : " not")
				+ " among allowed methods {}", accessControlRequestMethod,
				allowedMethods);
		return result;
	}
	...
	private boolean areHeadersAllowed(HttpServletRequest request) {
		...
		logger.debug("{} is {}", ACCESS_CONTROL_REQUEST_HEADERS_HEADER,
		...
		logger.debug("Headers [{}] are" + (result ? "" : " not")
				+ " among allowed headers {}", accessControlRequestHeaders,
				allowedHeaders);
		return result;
	}
