...
if (typeof(am.ik) == 'undefined') {
    /**
     * namespace: am.ik
     */
    am.ik = {};
}
if (typeof(am.ik.categolj) == 'undefined') {
    /**
     * namespace: am.ik.categolj
     */
    am.ik.categolj = {};
}
if (typeof(am.ik.categolj.entry) == 'undefined') {
    /**
     * namespace: am.ik.categolj.entry
     */
    am.ik.categolj.entry = {};
}
...
am.ik.categolj.entry.Form = {
    ...
    appendUploadRow: function(obj, uploadDir, file) {
        var path = am.ik.categolj.entry.Form.contextRoot +
        ...
        obj.append("<tr id='row" + id +
                "'><td onclick='am.ik.categolj.entry.Form.focusPath(" + id +
                ...
                "onclick='am.ik.categolj.entry.Form.insertUploadHtml(" + id +
                ")' /> <input type='button' class='btn btn-danger' " +
                "value='DELETE' " +
                "onclick='am.ik.categolj.entry.Form.deleteFile(" +
                id + ")' /></td></tr>");
    },

    deleteFile: function(id) {
        if (confirm('Are you sure to delete?')) {
            $.post(am.ik.categolj.entry.Form.contextRoot +
                   'upload/delete/' + id, function(res) {
                if (res.res == 'ok') {
                    $('#row' + id).fadeOut();
                } else {
                    alert('delete failed');
    ...
    search: function(target) {
        ...
        div.html('<p><img src="' + am.ik.categolj.entry.Form.contextRoot +
                'resources/images/ajax-loader.gif"></p>');
        $.get(am.ik.categolj.entry.Form.contextRoot +
                'apa/search/' + target, function(res) {
            if (res.error) {
                div.html("<div class='alert alert-error'>" +
                        '<p><strong>Error!</strong>: ' +
                        res.error + '</p>' +
                        '<p>Please check properties whose key starts with ' +
                        '<code>categolj.aws</code> in ' +
                        '<code>categolj.properties</code></p></div>');
            } else {
                var result = '<table class="' +
                'table table-striped table-bordered table-condensed">' +
                '<tr><th>TITLE</th><th>IMG</th><th>DATE</th>' +
                '<th>AUTHOR</th><th>ACTION</th></tr>';
                var products = res.products;
                for (var i = 0; i < products.length; i++) {
                    var product = products[i];
                    result = result + "<td><a href='" + product.link + "'>" +
                        product.title + '</a></td>' +
                        "<td><a href='" + product.imageLink +
                        "'><img width='50' src='" + product.imageLink +
                        "'></a></td>" +
                        '<td>' + product.publicationDate + '</td>' +
                        '<td>' + product.authors + '</td>' +
                        "<td><button class='btn btn-info' onclick=" +
                        "'am.ik.categolj.entry.Form.insertProductHtml(" +
    ...
    insertProductHtml: function(product) {
        var html = '<a href="' + product.link +
            ' "><img src="' + product.imageLink +
            '" title="' + product.title + '" alt="' +
            product.title + '"></a>';
            var body = $('#wmd-input');
        var text = body.text() + '\n' + html;
        body.text(text);
    ...
    init: function(contextRoot) {
        ...
        $('#upload-file').change(function() {
            $(this).upload(contextRoot + 'upload/post', function(res) {
                if (res.res == 'ok') {
                    am.ik.categolj.entry.Form.
                    appendUploadRow($('#upload-result'),
                                    res.uploadDir, res.file);
                } else {
                    alert('upload failed');
                }
                $('#upload-file').val('');
        ...
        $('#search-product-btn').click(function() {
            ...
            if (term.val().length > 0) {
                var chk = $('input[name=searchType]:checked');
                var target = chk.val() + '/' + term.val();
